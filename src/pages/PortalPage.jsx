...
const PortalPage = () => {
    ...
    if (sessionToken && view === 'dashboard') {
      if (sessionToken === 'demo-token') {
        ...
        const demoNodes = sortNodes([
          ...
          { id: 'demo-us2', name: 'usa-east-1', country: 'United States', countryCode: 'US', city: 'Orangeburg', status: 'ONLINE', load: 0 },
  ...
  const switchVpnNode = async (sid, nid, country) => { toast({ title: 'Switching...', description: `Connecting to ${country}...` }); try { const res = await fetch(`${API_BASE}/portal/vpn-switch`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...hdrs() }, body: JSON.stringify({ serviceId: sid, targetNodeId: nid }) }); const data = await res.json(); if (data.success) { setActiveNodeId(nid); toast({ title: 'Server Switched', description: 'Download the new config file and import it into WireGuard to connect to this server.', duration: 8000 }); loadServices(sessionToken, codeHash); } } catch (err) {} };
  const downloadVpnConfig = async (sid) => { try { const res = await fetch(`${API_BASE}/portal/vpn-config/${sid}`, { headers: hdrs() }); if (!res.ok) throw new Error(); const blob = await res.blob(); const node = vpnNodes.find(n => n.id === activeNodeId) || vpnNodes[0]; const sn = {'United Kingdom':'UK','United States':'US','Netherlands':'NL','Australia':'AU','India':'IN','Singapore':'SG','Canada':'CA'}; const cc = node ? (sn[node.country] || 'VN') : 'VN'; const rnd = Math.random().toString(36).substring(2,6); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'PP_' + cc + '_' + rnd + '.conf'; a.click(); URL.revokeObjectURL(a.href); toast({ title: 'Config Downloaded' }); } catch (err) { toast({ title: 'Not Available', variant: 'destructive' }); } };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="w-full max-w-sm relative z-10">
        <div className="bg-[#0a1120]/95 backdrop-blur-2xl p-6 md:p-8 border border-[#3affc2]/20 shadow-[0_0_60px_rgba(58,255,194,0.15)] flex flex-col items-center text-center relative overflow-hidden rounded-2xl">
          ...
          <form onSubmit={handleConnect} className="w-full flex flex-col gap-3 mb-6 relative z-20">
            <div className="relative group mb-3"><div className="absolute -inset-0.5 bg-gradient-to-r from-[#3affc2] to-[#0ea5e9] rounded-lg blur opacity-25 group-hover:opacity-40 transition duration-500" /><Input ref={inputRef} type="text" placeholder="Enter Access Code" value={accessCode} onChange={(e) => setAccessCode(e.target.value.replace(/\s+/g, ""))} className="relative w-full h-11 bg-[#050b14] border border-white/10 text-white placeholder:text-gray-500 text-center text-base rounded-lg focus-visible:ring-1 focus-visible:ring-[#3affc2]/50 focus-visible:border-[#3affc2] transition-all" style={{ ...mono, letterSpacing: '3px' }} /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-[#0a1120] border border-white/5 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button onClick={() => showPayment('Extend Pass', originalPurchase ? [{label:'Extend (' + originalPurchase.productSku + ')',price:'\u00a3' + parseFloat(originalPurchase.amount).toFixed(2),sku:originalPurchase.productSku,amount:Math.round(parseFloat(originalPurchase.amount) * 100),dmb:parseFloat(originalPurchase.amount) <= 4.50}] : [{label:'14 Days',price:'\u00a32.00',sku:'vpn-14',amount:200,dmb:true},{label:'30 Days',price:'\u00a33.50',sku:'vpn-30',amount:350,dmb:true}])} className="h-9 bg-[#3affc2]/15 border border-[#3affc2]/30 text-[#3affc2] hover:bg-[#3affc2]/25 hover:border-[#3affc2]/50 hover:shadow-[0_0_20px_rgba(58,255,194,0.2)] active:scale-95 text-[10px] rounded-xl flex items-center justify-center gap-1 transition-all animate-pulse" style={mono}><Zap className="w-3 h-3" />Extend</button>
              </div>
            </motion.div>
                ...
                {esimService.esimDetails ? (
                  <div className="space-y-3">
                    {esimService.esimDetails.qrData ? (
                      <div className="flex items-center gap-4"><div className="bg-white rounded-xl p-2.5 shadow-lg"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(esimService.esimDetails.qrData)}`} alt="eSIM QR" className="w-28 h-28" /></div><div className="flex-1"><p className="text-white/60 text-[11px] leading-relaxed">Scan this QR code in <span className="text-white">Settings → Cellular → Add eSIM</span> to install your data plan.</p><p className="text-[10px] text-white/50 mt-2" style={mono}>ICCID: <span className="text-blue-400">{esimService.esimDetails.iccid || '...'}</span></p></div></div>
                    ) : (
                      <div className="bg-[#050b14] border border-dashed border-white/10 rounded-xl p-5 text-center"><p className="text-white/70 text-sm mb-1" style={mono}>QR pending activation</p><p className="text-[10px] text-white/50" style={mono}>ICCID: <span className="text-blue-400">{esimService.esimDetails.iccid || '...'}</span></p></div>
                    )}
