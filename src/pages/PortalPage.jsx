...
const PortalPage = () => {
  ...
  const downloadVpnConfig = async (sid) => { try { const res = await fetch(`${API_BASE}/portal/vpn-config/${sid}`, { headers: hdrs() }); if (!res.ok) throw new Error(); const blob = await res.blob(); const cd = res.headers.get('content-disposition'); const fn = cd && cd.match(/filename="?([^"]+)"?/) ? cd.match(/filename="?([^"]+)"?/)[1] : 'PhantomPath_' + Math.random().toString(36).substring(2,5) + '.conf'; const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = fn; a.click(); URL.revokeObjectURL(a.href); toast({ title: 'Config Downloaded' }); } catch (err) { toast({ title: 'Not Available', variant: 'destructive' }); } };
