...
const PortalPage = () => {
  ...
  const downloadVpnConfig = async (sid) => { try { const res = await fetch(`${API_BASE}/portal/vpn-config/${sid}`, { headers: hdrs() }); if (!res.ok) throw new Error(); const blob = await res.blob(); const node = vpnNodes.find(n => n.id === activeNodeId) || vpnNodes[0]; const sn = {'United Kingdom':'UK','United States':'US','Netherlands':'NL','Australia':'AU','India':'IN','Singapore':'SG','Canada':'CA'}; const loc = node ? (sn[node.country] || node.country.substring(0,2)) + '_' + node.city : 'VPN'; const rnd = Math.random().toString(36).substring(2,5); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'PP_' + loc + '_' + rnd + '.conf'; a.click(); URL.revokeObjectURL(a.href); toast({ title: 'Config Downloaded' }); } catch (err) { toast({ title: 'Not Available', variant: 'destructive' }); } };
