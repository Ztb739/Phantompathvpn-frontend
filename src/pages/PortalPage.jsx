...
const PortalPage = () => {
  ...
  const downloadVpnConfig = async (sid) => { try { const res = await fetch(`${API_BASE}/portal/vpn-config/${sid}`, { headers: hdrs() }); if (!res.ok) throw new Error(); const blob = await res.blob(); const node = vpnNodes.find(n => n.id === activeNodeId) || vpnNodes[0]; const loc = node ? ((SHORT_NAME[node.country] || node.country).replace(/ /g,'_') + '_' + node.city).replace(/[^a-zA-Z0-9_]/g,'') : 'VPN'; const rnd = Math.random().toString(36).substring(2,5); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'PhantomPath_' + loc + '_' + rnd + '.conf'; a.click(); URL.revokeObjectURL(a.href); toast({ title: 'Config Downloaded' }); } catch (err) { toast({ title: 'Not Available', variant: 'destructive' }); } };
