...
const packData = {
  '1-week': [
    { id: '1w-1g', data: '1GB', duration: '7 Days', basePrice: 8.99, highlight: false, baseDesc: 'Quick trip data' },
    { id: '1w-5g', data: '5GB', duration: '7 Days', basePrice: 15.49, highlight: false, baseDesc: 'Better value' },
    { id: '1w-10g', data: '10GB', duration: '7 Days', basePrice: 21.49, highlight: true, baseDesc: 'Most popular', badgeText: 'BEST VALUE' },
    { id: '1w-50g', data: '50GB', duration: '7 Days', basePrice: 43.49, highlight: false, baseDesc: 'Ultimate data pack' },
  ],
  '2-weeks': [
    { id: '2w-1g', data: '1GB', duration: '14 Days', basePrice: 15.99, highlight: false, baseDesc: 'Quick trip data' },
    { id: '2w-5g', data: '5GB', duration: '14 Days', basePrice: 25.49, highlight: false, baseDesc: 'Better value' },
    { id: '2w-10g', data: '10GB', duration: '14 Days', basePrice: 35.49, highlight: true, baseDesc: 'Most popular', badgeText: 'BEST VALUE' },
    { id: '2w-50g', data: '50GB', duration: '14 Days', basePrice: 72.99, highlight: false, baseDesc: 'Ultimate data pack' },
  ],
  '1-month': [
    { id: '1m-10g', data: '10GB', duration: '30 Days', basePrice: 36.99, highlight: false, baseDesc: 'Better value' },
    { id: '1m-20g', data: '20GB', duration: '30 Days', basePrice: 62.99, highlight: true, baseDesc: 'Most popular', badgeText: 'BEST VALUE' },
    { id: '1m-50g', data: '50GB', duration: '30 Days', basePrice: 109.99, highlight: false, baseDesc: 'Ultimate data pack' },
  ]
};
...
const PackCard = ({ plan, activeTab, dataCountry, handleBuy }) => {
  ...
  const getDynamicDesc = () => {
    ...
    if (plan.data === '50GB') {
      return `${plan.baseDesc} — lowest cost (~£${perGb}/GB)`;
    } else {
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col p-5 rounded-xl border transition-all duration-300 group hover:-translate-y-1 hover:shadow-2xl overflow-visible h-full mb-8 md:mb-0 ${plan.highlight
          ? 'bg-[#0f172a] border-[#2dd4bf] shadow-[0_4px_30px_rgba(45,212,191,0.15)]'
          : 'bg-[#1e293b] border-white/5 hover:border-white/20 shadow-lg'
        }`}
    >
      {plan.highlight && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#2dd4bf] text-[#020617] text-[10px] sm:text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg whitespace-nowrap z-20">
          <Zap className="w-3 h-3 fill-current" /> {plan.badgeText || 'BEST VALUE'}
        </div>
      )}

      <div className="shrink-0 mb-3">
        <h3 className={`font-bold text-lg mb-1 ${plan.highlight ? 'text-[#2dd4bf]' : 'text-white'}`}>
          {plan.data} Data Pack
        </h3>
        <p className="text-xs text-gray-400 mb-2">{dataCountry} • {plan.duration}</p>
        <p className={`text-xs font-medium mb-3 pb-3 border-b border-white/5 ${plan.highlight ? 'text-[#2dd4bf]/80' : 'text-gray-400'}`}>
          {getDynamicDesc()}
        </p>
      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 mb-4 flex flex-col gap-4">
        <ul className="space-y-2 shrink-0 pt-1">
          <li className="flex items-start gap-2 text-xs text-gray-300">
            <Check className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${plan.highlight ? 'text-[#2dd4bf]' : 'text-[#38bdf8]'}`} />
            <span>eSIM Included</span>
          </li>
          <li className="flex items-start gap-2 text-xs text-gray-300">
            <Check className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${plan.highlight ? 'text-[#2dd4bf]' : 'text-[#38bdf8]'}`} />
            <span>High-Speed Data ({plan.data})</span>
          </li>
          <li className="flex items-start gap-2 text-xs text-gray-300">
            <Check className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${plan.highlight ? 'text-[#2dd4bf]' : 'text-[#38bdf8]'}`} />
          ...
          <div className="p-3 rounded-xl bg-black/30 border border-white/5">
            <div className="flex items-center justify-between cursor-pointer" onClick={(e) => { e.preventDefault(); onToggle(); }}>
              <span className="text-xs font-semibold flex items-center gap-2 text-gray-200">
                <Phone className="w-3.5 h-3.5 text-[#38bdf8]" /> Add Virtual Number
              </span>
              <div className={`w-10 h-5 rounded-full p-0.5 transition-colors ${expanded ? 'bg-[#2dd4bf]' : 'bg-gray-600'}`}>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-2.5 mt-3 overflow-visible">
                  <div>
                    ...
                    <CustomSelect
                      ...
                      focusBorderColor="border-[#2dd4bf]"
                    />
                  </div>

                  <div className="text-[10px] text-[#2dd4bf] bg-[#2dd4bf]/10 p-2 rounded-lg border border-[#2dd4bf]/20 font-medium flex justify-between">
                    <span>Included:</span>
      ...
      <div className="mt-auto shrink-0 pt-2 w-full flex flex-col gap-3">
        ...
        <Button
          ...
          className={`w-full font-bold rounded-lg h-10 text-xs uppercase tracking-wider transition-all duration-300 glow-phantom glow-phantom-hover mt-auto ${expanded && numberCountry === 'United States' && (!usaConsent || !userMobile.trim())
              ? 'bg-white/5 text-gray-600 border border-white/10 cursor-not-allowed'
              : plan.highlight
                ? 'bg-[#2dd4bf] text-[#020617] hover:bg-white hover:text-black'
                : 'bg-[#38bdf8]/10 text-[#38bdf8] border border-[#38bdf8]/30 hover:bg-[#38bdf8] hover:text-white'
            }`}
        >
          Get Data Pack
        </Button>
        <p className="text-[10px] text-gray-500 text-center leading-tight">
          🛡️ 18+ REQUIREMENT: By proceeding via PayPal, Stripe, Apple Pay, or Google Pay, you agree to the immediate provision of digital data and acknowledge you waive your statutory 14-day right to cancel once the eSIM is provisioned.
        </p>
      </div>
    </motion.div>
  );
...
const TravelPacks = () => {
  ...
  return (
    <section id="travel-packs" className="py-12 lg:py-16 px-4 bg-[#050b14]">
      <div className="container mx-auto max-w-7xl">
        <div className="text-center mb-8">
          <h2 className="text-2xl md:text-4xl font-black mb-3 text-white tracking-tight">Travel Packs - Stay connected abroad with our high-speed travel eSIMs.</h2>
          <p className="text-[#2dd4bf] font-semibold text-base">No subscription. No commitment.</p>
            ...
            <div className="inline-flex bg-[#1e293b] p-1 rounded-xl border border-white/5 shadow-lg">
              {[
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-lg text-sm font-bold transition-all duration-300 ${activeTab === tab.id
                      ? 'bg-[#2dd4bf] text-[#020617] shadow-md'
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-4 gap-0 md:gap-8 lg:gap-6 justify-center max-w-6xl mx-auto items-stretch">
          {packData[activeTab].map((plan) => (
            <PackCard
              key={plan.id}
              plan={plan}
              activeTab={activeTab}
        ...
        <div className="max-w-4xl mx-auto mt-10 space-y-3">
          <div className="text-center text-[11px] text-gray-400 bg-[#1e293b]/50 p-4 rounded-xl border-l-4 border-l-[#2dd4bf] border-white/5 space-y-2">
